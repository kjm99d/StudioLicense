...
// @Summary 라이선스 비활성화
// @Description 디바이스에서 라이선스를 비활성화합니다
// @Tags 클라이언트 - 라이선스
// @Accept json
// @Produce json
// @Param request body models.DeactivateRequest true "비활성화 정보"
// @Success 200 {object} models.APIResponse "비활성화 성공"
// @Failure 400 {object} models.APIResponse "잘못된 요청"
// @Failure 404 {object} models.APIResponse "라이선스 또는 디바이스 없음"
// @Failure 500 {object} models.APIResponse "서버 에러"
// @Router /api/license/deactivate [post]
func DeactivateLicense(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT id FROM licenses WHERE license_key = ?"
	err := database.DB.QueryRow(query, req.LicenseKey).Scan(&licenseID)
	...
	result, err := database.DB.Exec(updateQuery, models.DeviceStatusDeactivated, now, licenseID, fingerprint)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
